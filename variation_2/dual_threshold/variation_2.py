...
def main (argv):
    ...
    columns_mpdock = ['pi_score', 'Num_intf_residues', 'int_solv_en', 'int_area', 'hb']
    columns_lis = [ 'iptm', 'pi_score', 'average pae score', 'Polar', 'Hydrophobhic',]
    ...
    if lia_df.empty:
        ...
        rename_dict = {
            ...
            'Hydrophobhic': 'norm_Hydrophobhic', 
            'Polar': 'norm_Polar'
        }
    ...
    if not normdock_df.empty:
       ...
       birch.fit(normdock_df[['norm_pi', 'norm_num', 'norm_int', 'norm_int_a', 'norm_hb']])
    ...
    if not normlia_df.empty:
       birch = Birch(branching_factor=50, threshold=0.3, n_clusters=4)
       birch.fit(normlia_df[['norm_iptm', 'norm_pi', 'norm_pae', 'norm_Hydrophobhic', 'norm_Polar']])       
       labels = birch.labels_
    ...
    try:
        path_output = '/Users/castroverdeac/Desktop/codes_for_AF/actual_frag_af/data/variation2_predictions'
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            normdock_df.to_excel(writer, sheet_name='normalized mpDockQ Contact data', index=False)
            normlia_df.to_excel(writer, sheet_name='normalized LIA LIS data', index=False)
            lis_excel.to_excel(writer, sheet_name='LIS LIA scores', index=True)
